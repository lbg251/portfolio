...
Inputs are sequences of integers in the range [0,11], and are either noise (context and completions drawn randomly) or signal (two of the context's integers are drawn in the range [1,4], the final token is 0, and the completions are generated by an arbitrary map). We implemented a 1-layer, attention-only toy model with one-hot (un)embeddings using **PyTorch**, which solves this task with perfect accuracy. We find two possible mechanisms for superposition, controlled by the number of heads and the head dimension, respectively. 
